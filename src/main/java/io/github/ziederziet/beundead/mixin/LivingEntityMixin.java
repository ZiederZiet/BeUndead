...
public abstract class LivingEntityMixin {
    ...
    public void hurt(DamageSource damageSource, float f, CallbackInfoReturnable<Boolean> info){
        ...
        if (!livingEntity.getLevel().isClientSide()){
            ModEvents.AfterDamageEvent(livingEntity, damageSource, f);
        }
    }
                ...
                if (type != null && type.burnsInTheSun() && BeUndeadHelper.isSunBurnTick(player)){
                    ...
                    if (!itemstack.isEmpty()) {
                        if (itemstack.isDamageableItem()) {
                            itemstack.hurtAndBreak(player.getRandom().nextInt(2), livingEntity, livingEntity1 -> {
                                livingEntity1.broadcastBreakEvent(EquipmentSlot.HEAD);
                            });
    ...
    public void canBeAffected(MobEffectInstance pEffectInstance, CallbackInfoReturnable<Boolean> info) {
        LivingEntity livingEntity = (LivingEntity) (Object)this;
        if ((livingEntity instanceof Player player && !BeUndeadHelper.isHuman(player)) && (pEffectInstance.getEffect().equals(MobEffects.REGENERATION) || pEffectInstance.getEffect().equals(MobEffects.POISON) || pEffectInstance.getEffect().equals(MobEffects.HUNGER))){
            info.setReturnValue(false);
            info.cancel();
        }
    }
    ...
    protected void dropExperience(DamageSource damageSource, CallbackInfo info) {
        if (damageSource.getEntity() instanceof Player player && !player.getLevel().isClientSide() && !BeUndeadHelper.isHuman(player)){
            info.cancel();
            ...
            if (!(thisEntity instanceof Monster)){
                ...
                else if (thisEntity.shouldDropExperience() && thisEntity.getLevel().getGameRules().getBoolean(GameRules.RULE_DOMOBLOOT)){
    ...
    public void eat(Level level, ItemStack itemStack, CallbackInfoReturnable<ItemStack> info){
    ...
    private void addSmiteDamageRedirect(LivingEntity instance, DamageSource damageSource, float f) {
        if (damageSource.getEntity() instanceof LivingEntity attacker && (Object)this instanceof Player target && !BeUndeadHelper.isHuman(target)) {
            ItemStack weapon = attacker.getMainHandItem();
            if (weapon != null){
                int smiteLevel = EnchantmentHelper.getItemEnchantmentLevel(Enchantments.SMITE, weapon);

                if (smiteLevel > 0) {
                    float bonus = smiteLevel * 2.5F;
                    actuallyHurt(damageSource, f + bonus);
                }
            }
        }
    ...
    protected void onEffectUpdated(MobEffectInstance mobEffectInstance, boolean bl, Entity entity, CallbackInfo info){
        if (mobEffectInstance.getEffect() == BeUndead.INFECTED_EFFECT){
            LivingEntity livingEntity = (LivingEntity)(Object)this;
            if (!livingEntity.getLevel().isClientSide()){
                BeUndeadHelper.addedInfectionEffect(livingEntity);
            }
        }
    }
}