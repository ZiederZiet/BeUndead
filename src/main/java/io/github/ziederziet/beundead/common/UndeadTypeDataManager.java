            ...
            try {
                ...
                if (unserializedMobEffects != null){
                    ...
                    for (int i = 0; i < mobEffects.length; i++) {
                        Optional<Holder.Reference<MobEffect>> optional = BuiltInRegistries.MOB_EFFECT.get(ResourceLocation.parse(unserializedMobEffects[i].mobEffectId()));
                ...
                if (data.damageTypes() != null && BeUndead.getServer() != null){
                    ...
                    for (String damageTypeId : data.damageTypes()){
                        Optional<Holder.Reference<Registry<DamageType>>> reg = access.get(Registries.DAMAGE_TYPE);
                        Optional<Holder.Reference<DamageType>> optional = reg.get().value().get(ResourceLocation.parse(damageTypeId));
                        if (optional.isPresent()){
